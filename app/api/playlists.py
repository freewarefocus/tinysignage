...
def _playlist_hash(items: list[PlaylistItem]) -> str:
    ...
    for item in sorted(items, key=lambda i: i.order):
        ...
        parts.append(
            ...
            f":{item.transition_type}:{item.transition_duration}:{item.duration}:{item.object_fit}:{item.effect}"
        )
...
async def update_playlist_item(
    ...
    allowed = {"transition_type", "transition_duration", "duration", "object_fit", "effect"}
    changes = {}
    for key, value in body.items():
        if key in allowed:
